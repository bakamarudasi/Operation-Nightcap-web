...
旧仕様（上記 BattleScreen 節）の縦積みレイアウトを廃止し、
**3カラムレイアウト + 左カラム立ち絵**に全面改修。

### レイアウト概要

```
┌──────────────────────────────────────────────────────────────┐
│ [ヘッダー]  ロドスバー         R.3/12           龍 3,200     │
├──────────┬──────────────────────────────┬────────────────────┤
│          │ [相手バー]                    │ STATUS PANEL       │
│          │ 🔥 ブレイズ  燃え盛る太陽      │                    │
│  立ち絵   │ 酔い ██████░░ べろべろ (6/10) │ ドクター状態        │
│  (大)    │                              │  ほろ酔い           │
│ 280px    │ ┌─────────┐                  │                    │
│          │ │ OPP  VS  PL │              │ 酔いレベル          │
│  🔥      │ │ 3Dフリップ   │              │ ██░░ 3/10          │
...
│          │                              │ 戦績  2勝 1敗       │
│ ブレイズ  │ [セリフバー]                  │                    │
│ べろべろ  │ ブレイズ：おい…暑くないか…？   │ デッキ 🃏 残り 6    │
│          │                              │ 相手   🎴 残り 5    │
│          │ [ドクター酔いゲージ]           │                    │
│          │ ██░░░░ ほろ酔い (2/10)        │ 直前ラウンド        │
├──────────┴──────────────────────────────┤ 自分: 🍺 ビール    │
│   🍺     🍷     🥜     💋              │ 相手: 🍖 焼き鳥    │
│  ビール   ワイン   ナッツ   キス           │ 結果: 勝ち          │
│   1      2     回復1   特殊             │                    │
│          (手札エリア + ツールチップ)       │                    │
└──────────────────────────────────────────┴────────────────────┘
```
...
| カラム | 幅 | 内容 |
|--------|------|------|
| 左（`.battle-left`） | 280px 固定 | キャラ立ち絵 + 名前 + 酔い状態ラベル |
| 中央（`.battle-center`） | flex: 1 | 相手バー → フィールド → セリフ → ドクターゲージ |
| 右（`.status-panel`） | 200px 固定 | ドクター状態・酔い・戦績・デッキ残り・直前ラウンド |

### 左カラム — 立ち絵エリア詳細

#### コンテナ `.char-portrait`
- サイズ: **220 x 340px**
- 背景: `rgba(20,12,6,0.6)` + `inset box-shadow`
- 角丸: 12px
- キャラのテーマカラーに応じた底部グロウ（CSS変数 `--char-glow`）

#### 現状の表示
- emoji アイコンを `6rem` で大きく配置
- `drop-shadow` で浮かせる

#### 将来の立ち絵画像差し替え
- `.char-portrait img` のスタイル定義済み（`object-fit: cover`、`object-position: center top`）
- 画像を `<img src={...}>` で配置するだけで切り替わる構造
- アセットパス想定: `src/assets/characters/{id}/portrait.webp`

#### 酔い演出（立ち絵）
| 酔いLv | CSS class | 効果 |
|--------|-----------|------|
| 0 | なし | 通常表示 |
| 1 | `drunk-1` | `brightness(1.05) saturate(1.1)` |
| 2 | `drunk-2` | `brightness(1.1) saturate(1.2)` + 揺れアニメ 3s |
| 3 | `drunk-3` | `brightness(1.15) saturate(1.3) hue-rotate(-5deg)` + 揺れ 2s |
| 4 | `drunk-4` | `brightness(0.9) saturate(1.4) hue-rotate(-10deg)` + 崩れ落ちアニメ |

#### 赤み演出 `.char-portrait-flush`
- `radial-gradient` で頬に赤み
- 酔い値に比例して `rgba(200,40,40, 0〜0.3)` が変化
- `useMemo` で計算、`transition: 1s` でなめらかに遷移

#### リアクション `.char-reaction`
- 右上に絵文字ポップ（`😵` / `😏` など）
- `scale(0) → scale(1)` の spring アニメ
- 2秒後に自動消去

### 中央カラム — ゲームエリア

#### 相手バー `.opponent-bar`
- 48px ミニポートレート + 名前 + サブタイトル + 酔いゲージ
- ゲージ: 8px 高、赤グラデーション `#884422 → #cc5533 → #ff6644`

#### フィールド `.battle-field` → `.field-table`
- `aspect-ratio: 16/9`、max-width 600px
- `border: 1px dashed` の破線枠 + 微弱 radial-gradient

#### 3Dカード `.card-slot` → `.card-3d`
- **サイズ**: 110 x 154px
- **perspective**: 700px
- **裏面**: 木目ダーク + 和柄パターン（wave / check）+ 金枠 + 「？」
- **表面**: `rotateY(180deg)` で裏側に配置、`.flipped` で `rotateY(180deg)` → 表が見える
- **Slam演出**: `translateY(-60px) scale(1.3) rotate(-5deg)` → 着地
- **画面シェイク**: `screenShake` keyframes、0.4s

#### VSバッジ
- `font-size: 2.2rem`、赤色、`::before` `::after` で左右に金線

#### セリフバー `.dialogue-bar`
- 左に金の 3px ボーダー
- タイピングエフェクト: 30ms/文字の `setInterval`

#### プレイヤーゲージ
- 青グラデーション `#336688 → #4488aa → #55aacc`

### 右カラム — ステータスパネル

| セクション | 表示内容 |
|-----------|---------|
| ドクター状態 | 酔い段階テキスト（色変化あり） |
| 酔いレベル | ミニゲージ + 数値 |
| 戦績 | 勝ち / 負け カウント |
| デッキ | 自分の残りカード数 |
| 相手デッキ | 相手の残りカード数 |
| 直前ラウンド | 自分・相手の出したカード + 勝敗 |
...
- タイプ別ボーダーカラー:
  ...
  - food: `#448844` 緑
  - chug: `#665588` 紫
  - harassment: `#884466` ピンク
- ホバー: `translateY(-28px) scale(1.1)`、z-index 60
- 選択時: `translateY(-32px) scale(1.12)`、金枠グロウ
- ツールチップ: ホバー時にカード上部にポップ（カード名 + 説明）
- 処理中の他カード: `opacity: 0.35`、`grayscale(0.5)`

### 背景レイヤー（重ね順）

| レイヤー | z-index | 内容 |
|---------|---------|------|
| `.battle-bg-gradient` | 0 | 暖色 radial-gradient + 縦 linear-gradient |
| `.battle-bg-noise` | 0 | SVG feTurbulence ノイズ、opacity 0.03 |
| `.battle-bg-table` | 0 | 92deg 木目テクスチャ |
| `.battle-drunk-blur` | 1 | 酔い度連動 backdrop-filter blur（max 4px） |
| `.battle-particles` | 0 | 金色パーティクル 12個、10〜25s で上昇 |

### 酔い演出（画面全体）

プレイヤーの酔い度に応じて画面全体に影響：

| 効果 | 制御 | 最大値 |
|------|------|--------|
| 画面ブラー | `backdrop-filter: blur()` | 4px |
| 画面傾き | `transform: rotate()` | ±1.5deg |

`useEffect` で `playerDrunk` 変化時に ref で直接 DOM 操作。
`transition: 1.5s` でゆっくり変化。

### 効果音（Web Audio API）

画像/音声アセットなしで動作するよう、Web Audio API で合成：

| 効果音 | トリガー | 実装 |
|--------|---------|------|
| slam | カードをフィールドに出した瞬間 | 正弦波 180Hz→50Hz + ノイズバースト |
| flip | 相手カードが表になった瞬間 | 正弦波 2000Hz、短い click 音 |

### 酔い段階テーブル（表示用）

BattleScreen 内で `DRUNK_STAGES` 定数として定義：

| 酔い値 | テキスト | CSSクラス | テキストカラー |
|--------|---------|-----------|--------------|
| 0 | シラフ | `drunk-sober` | `var(--paper)` 白 |
| 1 | ほろ酔い | `drunk-tipsy` | `#ddaa66` 黄 |
| 2-3 | 酔い | `drunk-good` | `#ee8844` 橙 |
| 4-6 | べろべろ | `drunk-done` | `#ee5533` 赤橙 |
| 7-9 | 泥酔 | `drunk-wasted` | `#ff3333` 赤 |
| 10 | 潰れ | `drunk-gone` | `#ff2222` + pulse 点滅 |

### ラウンドシーケンス（タイミング）

```
t=0ms     カードクリック → selectCard()
t=500ms   handlePlayCard() 自動発火
          ├── プレイヤーカード: slam + flip + shake
t=1300ms  ├── 相手カード: slam
t=1700ms  │   └── 相手カード: flip
t=2100ms  ├── セリフ更新 + リアクション絵文字
          ├── (CGイベントあれば +1000ms で showCG)
t=3600ms  └── 勝敗チェック or 次ラウンド準備
              ├── テーブルリセット
              ├── drawHands()
              └── 相手セリフ更新
```
...
- `backdrop-filter: blur(4px)` + 85% 暗転
- カード: 金枠、木目グラデーション、`slideUp` アニメーション
- 報酬表示: 金色テキスト `+{amount} 龍門幣`
- 「店に戻る」ボタン → `setScreen('title')`

### ファイル構成

| ファイル | 役割 |
|---------|------|
| `src/components/BattleScreen.tsx` | 画面コンポーネント（ロジック + レンダリング） |
| `src/styles/battle.css` | 全スタイル（1109行） |
| `src/components/Card.tsx` | 他画面用カードコンポーネント（対戦画面では不使用） |

### 未実装・今後の拡張

| 項目 | 状態 | 備考 |
|------|------|------|
| 立ち絵画像 | 未実装 | emoji 表示、`<img>` 差し替え可能な構造 |
| ラウンド開始演出 | CSS定義済み | `.round-overlay` + keyframes あり、TSX 未接続 |
| 結果ポップアップ（フィールド上） | CSS定義済み | `.card-result-popup`、TSX 未接続 |
| カード飛行アニメ | CSS定義済み | `.hand-card.playing` の `cardToField`、`--tx`/`--ty` 未設定 |
| 相手カードの特定 | 暫定実装 | メッセージ内カード名マッチで推測。`playRound` の戻り値に相手カードID追加が望ましい |
| BGM / SE ファイル | 未実装 | Web Audio 合成音で仮対応中 |
